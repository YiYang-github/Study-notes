...
class MultiTaskNet(nn.Module):
    ...
    def init_shared_user_and_item_embeddings(self, num_users, num_items, embedding_dim):
        ...
        # U = torch.nn.Embedding(num_users, embedding_dim)
        # Q = torch.nn.Embedding(num_items,embedding_dim)
        ### END CODE HERE ###
        return U, Q
    ...
    def init_separate_user_and_item_embeddings(self, num_users, num_items, embedding_dim):
        ...
        # U_reg = torch.nn.Embedding(num_users, embedding_dim)
        # U_fact = torch.nn.Embedding(num_users, embedding_dim)
        # Q_reg = torch.nn.Embedding(num_items,embedding_dim)
        # Q_fact = torch.nn.Embedding(num_items,embedding_dim)
        ### END CODE HERE ###
    ...
    def init_item_bias(self, num_users, num_items):
        ...
        # B = torch.nn.Embedding(num_items,1)
        # nn.init.constant_(B.weight, 0) 
        B = ZeroEmbedding(num_items,1)#ZeroEmbedding
        ### END CODE HERE ###
        return B
    ...
    def forward_with_embedding_sharing(self, user_ids, item_ids):
        ...
        # Regression head
        element_dot = u_i*q_j
        feat_int = torch.cat((u_i,q_j,element_dot),dim=1)
        for layer in self.mlp_layers:
            feat_int = layer(feat_int)

        score = feat_int.squeeze()

        # Matrix Factorization Head  
  
        mf = torch.sum(element_dot,dim=1).squeeze() + b_j.squeeze()
        predictions = mf  #(user_len,item_len)
    ...
    def forward_without_embedding_sharing(self, user_ids, item_ids):
        ...
        # Regression head
        # u_i,q_j= self.U_reg[user_ids], self.Q_reg[item_ids]
        u_i = self.U_reg(user_ids) 
        q_j = self.Q_reg(item_ids)  
        ...
        u_i = self.U_fact(user_ids) 
        q_j = self.Q_fact(item_ids) 
        element_dot = u_i*q_j
        mf = torch.sum(element_dot,dim=1).squeeze() + b_j.squeeze()
        predictions = mf
